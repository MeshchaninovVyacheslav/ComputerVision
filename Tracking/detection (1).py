...
def extract_detections(frame, min_confidence=0.6, labels=None):
    ...
    h, w, _ = frame.shape
    #frame_ = resize(frame.astype('float32'), (300,300))
    frame_ = resize(frame, (300, 300)) * 255.0
    ...
    predictions = model.predict(frame_.reshape((1,) + frame_.shape))
    result = bbox_util.detection_out(predictions, confidence_threshold=min_confidence)[0]
    ...
    for i, p in enumerate(result[:,1]):
        if p > min_confidence:
            indeces.append(i)
    results = result[indeces]
    '''
    # If label set is known, use it
    if labels is not None:
        result_labels = result[:, 0].astype(np.int32)
    ...
    results = np.array(result).reshape((-1,6))[:, [0,2,3,4,5]]
